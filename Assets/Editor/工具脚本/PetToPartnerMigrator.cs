...
public class PetToPartnerMigrator : EditorWindow
{
    [MenuItem("Tools/迁移宠物到伙伴")]
    public static void MigratePetsToPartners()
    {
        // 定义要迁移的宠物名称列表
        ...
        // 确保目标目录存在
        string targetDir = "Assets/GameConf/伙伴/UR";
        if (!AssetDatabase.IsValidFolder(targetDir))
        {
            AssetDatabase.CreateFolder("Assets/GameConf/伙伴", "UR");
        }
        ...
        foreach (string petName in petNames)
        {
            ...
            {
                // 源文件路径
                string sourcePath = $"Assets/GameConf/宠物/UR/{petName}.asset";
                ...
                // 加载源宠物配置
                PetConfig sourcePet = AssetDatabase.LoadAssetAtPath<PetConfig>(sourcePath);
                if (sourcePet == null)
                {
                    errors.Add($"无法加载宠物配置: {sourcePath}");
                    continue;
                }

                // 创建新的伙伴配置
                PartnerConfig newPartner = ScriptableObject.CreateInstance<PartnerConfig>();

                // 复制所有基础属性
                CopyBasePetConfig(sourcePet, newPartner);

                // 目标文件路径
                string targetPath = $"{targetDir}/{petName}.asset";

                // 创建新文件
                AssetDatabase.CreateAsset(newPartner, targetPath);

                // 删除原文件
                AssetDatabase.DeleteAsset(sourcePath);

                migratedCount++;
                Debug.Log($"成功迁移: {petName} 从宠物到伙伴");
            }
            catch (System.Exception e)
            {
                errors.Add($"迁移 {petName} 时出错: {e.Message}");
            }
        }
