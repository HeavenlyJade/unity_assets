...
namespace MiGame.Tools
{
	/// <summary>
	/// 翅膀商城商品描述批量生成工具
	/// 从 Scripts/配置exel/翅膀.json 读取翅膀的加成配置，匹配 GameConf/商城/翅膀商城 下的 ShopItemConfig，生成商品描述
	/// 规则：
	/// 商品描述 = "速度加成：星级X{速度系数}\n加速度：星级X{加速度系数}\n金币加成：星级X{金币系数}"
	/// </summary>
	public static class WingShopDescriptionGenerator
	{
		...
		public static void GenerateDescriptions()
		{
			try
			{
				...
				foreach (string guid in guids)
				{
					...
					{
						// 未匹配到的项仅提示，不报错
						// Debug.LogWarning($"未在翅膀配置中找到：{name}");
					}
				}
