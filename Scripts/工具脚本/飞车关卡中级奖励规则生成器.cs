...
namespace MiGame.Tools
{
    /// <summary>
    /// 飞车关卡中级奖杯奖励规则生成器
    /// 按照400万一个轮回的等差数列公式生成奖杯奖励规则
    /// 每个轮回：100万=50奖杯，200万=100奖杯，400万=150奖杯
    /// 实际触发距离为原始距离的75%，让玩家更容易获得奖励
    /// 默认生成5个轮次，可自定义轮次数量
    /// </summary>
    public class 飞车关卡中级奖励规则生成器 : MonoBehaviour
    {
        [Header("关卡配置选择")]
        [Tooltip("要配置的关卡配置")]
        ...
        public string 关卡配置路径 = "Assets/GameConf/关卡/飞行关卡中级.asset";
        
        [Header("生成配置")]
        [Tooltip("最短距离（米）")]
        public int 最短距离 = 1000000; // 100万米
        
        [Tooltip("最大距离（米）")]
        public int 最大距离 = 20000000; // 2000万米（支持5个轮次）
        ...
        [Tooltip("200万阶段的奖杯数量")]
        public int 二百万奖杯 = 100;
        
        [Tooltip("400万阶段的奖杯数量")]
        public int 四百万奖杯 = 150;
        ...
        #if UNITY_EDITOR
        /// <summary>
        /// Unity编辑器菜单项 - 打开飞车关卡中级奖励规则生成器窗口
        /// </summary>
        [MenuItem("Tools/关卡配置/飞车关卡中级奖杯奖励规则生成器")]
        public static void 打开生成器窗口()
        {
            var 窗口 = EditorWindow.GetWindow<飞车关卡中级奖励规则生成器窗口>("飞车关卡中级奖励规则生成器");
            窗口.Show();
        }
        
        /// <summary>
        /// Unity编辑器菜单项 - 快速生成飞车关卡中级奖励规则
        /// </summary>
        [MenuItem("Tools/关卡配置/快速生成飞车关卡中级奖励规则")]
        public static void 快速生成飞车关卡中级奖励规则()
        {
            // 查找飞车关卡配置
            ...
            {
                if (配置.关卡名称.Contains("飞行") && (配置.关卡名称.Contains("中级") || 配置.关卡名称.Contains("中级")))
                {
                    目标配置 = 配置;
                    break;
                }
            ...
            if (目标配置 == null)
            {
                EditorUtility.DisplayDialog("错误", "未找到飞车关卡中级配置！", "确定");
                return;
            }
            
            // 生成默认奖励规则
            var 生成器 = new 飞车关卡中级奖励规则生成器();
            生成器.目标关卡配置 = 目标配置;
            生成器.最短距离 = 100000; // 100万米
            生成器.最大距离 = 20000000; // 2000万米（支持5个轮次）
            生成器.轮次数量 = 5; // 生成5个轮次
            生成器.一百万奖杯 = 50;
            生成器.二百万奖杯 = 100;
            生成器.四百万奖杯 = 150;
            ...
            EditorUtility.DisplayDialog("成功", $"已为关卡 '{目标配置.关卡名称}' 生成并应用了 {生成器.生成的奖励规则.Count} 个中级奖励规则！", "确定");
        }
        
        /// <summary>
        /// Unity编辑器菜单项 - 批量生成所有中级关卡奖励规则
        /// </summary>
        [MenuItem("Tools/关卡配置/批量生成所有中级关卡奖励规则")]
        public static void 批量生成所有中级关卡奖励规则()
        {
            ...
            foreach (var 关卡配置 in 所有关卡配置)
            {
                ...
                {
                    // 只处理包含"飞车"和"中级"的关卡
                    if (关卡配置.关卡名称.Contains("飞车") && (关卡配置.关卡名称.Contains("中级") || 关卡配置.关卡名称.Contains("中级")))
                    {
                        生成器.目标关卡配置 = 关卡配置;
                        生成器.最短距离 = 100000; // 100万米
                        生成器.最大距离 = 20000000; // 2000万米（支持5个轮次）
                ...
                {
                    Debug.LogError($"为关卡 '{关卡配置.关卡名称}' 生成中级奖励规则时出错: {e.Message}");
                }
            }
            
            EditorUtility.DisplayDialog("完成", $"成功为 {成功数量} 个中级飞车关卡生成了奖励规则！", "确定");
        }
        #endif
        ...
        public void 生成奖励规则()
        {
            ...
            // 按照400万一个轮回的等差数列公式生成规则
            // 每个轮回：100万=50奖杯，200万=100奖杯，400万=150奖杯
            // 实际触发距离为原始距离的75%，让玩家更容易获得奖励
            // 生成指定轮次数量的规则
            for (int 轮回 = 0; 轮回 < 轮次数量; 轮回++)
            {
                int 轮回起始距离 = 轮回 * 4000000;
                
                // 100万阶段（每个轮回内）- 距离乘以0.75
                int 距离100万 = 轮回起始距离 + 1000000;
                int 实际距离100万 = (int)(距离100万 * 0.75f);
                if (实际距离100万 >= 最短距离 && 实际距离100万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={实际距离100万}", $"rule_{规则计数器:D4}", 一百万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成100万阶段规则: 原始距离={距离100万}, 实际距离>={实际距离100万}, 奖杯={一百万奖杯}");
                    }
                }
                
                // 200万阶段（每个轮回内）- 距离乘以0.75
                int 距离200万 = 轮回起始距离 + 2000000;
                int 实际距离200万 = (int)(距离200万 * 0.75f);
                if (实际距离200万 >= 最短距离 && 实际距离200万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={实际距离200万}", $"rule_{规则计数器:D4}", 二百万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成200万阶段规则: 原始距离={距离200万}, 实际距离>={实际距离200万}, 奖杯={二百万奖杯}");
                    }
                }
                
                // 400万阶段（每个轮回内）- 距离乘以0.75
                int 距离400万 = 轮回起始距离 + 4000000;
                int 实际距离400万 = (int)(距离400万 * 0.75f);
                if (实际距离400万 >= 最短距离 && 实际距离400万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={实际距离400万}", $"rule_{规则计数器:D4}", 四百万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成400万阶段规则: 原始距离={距离400万}, 实际距离>={实际距离400万}, 奖杯={四百万奖杯}");
                    }
                }
            }
            ...
            Debug.Log($"成功生成 {生成的奖励规则.Count} 个中级奖励规则");
        ...
        public void 应用规则到选中关卡()
        {
            ...
            Debug.Log($"已应用 {生成的奖励规则.Count} 个中级奖励规则到关卡: {目标关卡配置.关卡名称}");
        ...
        public void 应用规则到关卡配置()
        {
            ...
            foreach (var 关卡配置 in 关卡配置s)
            {
                if (关卡配置.关卡名称.Contains("飞车") && (关卡配置.关卡名称.Contains("中级") || 关卡配置.关卡名称.Contains("中级")))
                {
                    ...
                    Debug.Log($"已应用 {生成的奖励规则.Count} 个中级奖励规则到关卡: {关卡配置.关卡名称}");
        ...
        public void 预览生成的规则()
        {
            ...
            Debug.Log($"=== 预览生成的 {生成的奖励规则.Count} 个中级奖励规则 ===");
        ...
        public void 导出规则为JSON()
        {
            ...
            string json = JsonUtility.ToJson(new { 中级奖励规则 = 导出数据 }, true);
            Debug.Log($"中级奖励规则JSON:\n{json}");
        }
    ...
    /// 飞车关卡中级奖励规则生成器编辑器窗口
    /// </summary>
    public class 飞车关卡中级奖励规则生成器窗口 : EditorWindow
    {
        private 飞车关卡中级奖励规则生成器 生成器;
        private Vector2 滚动位置;
        
        private void OnEnable()
        {
            生成器 = new 飞车关卡中级奖励规则生成器();
        }
        
        private void OnGUI()
        {
            titleContent = new GUIContent("飞车关卡中级奖励规则生成器");
            
            EditorGUILayout.BeginVertical();
            
            // 标题
            EditorGUILayout.LabelField("飞车关卡中级奖杯奖励规则生成器", EditorStyles.boldLabel);
            ...
            生成器.二百万奖杯 = EditorGUILayout.IntField("200万阶段奖杯", 生成器.二百万奖杯);
            生成器.四百万奖杯 = EditorGUILayout.IntField("400万阶段奖杯", 生成器.四百万奖杯);
            ...
            {
                EditorGUILayout.LabelField($"生成结果: {生成器.生成的奖励规则.Count} 个中级规则", EditorStyles.boldLabel);
