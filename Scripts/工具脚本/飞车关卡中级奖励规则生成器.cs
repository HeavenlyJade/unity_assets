...
namespace MiGame.Tools
{
    ...
    public class 飞车关卡中级奖励规则生成器 : MonoBehaviour
    {
        ...
        [Header("生成配置")]
        [Tooltip("最短距离（米）")]
        public int 最短距离 = 150000; // 15万米
        
        [Tooltip("最大距离（米）")]
        public int 最大距离 = 8000000; // 800万米（支持5个轮次）
        ...
        [Tooltip("40万阶段的奖杯数量")]
        public int 四十万奖杯 = 100;
        
        [Tooltip("80万阶段的奖杯数量")]
        public int 八十万奖杯 = 150;
        ...
        public static void 快速生成飞车关卡中级奖励规则()
        {
            ...
            生成器.目标关卡配置 = 目标配置;
            生成器.最短距离 = 150000; // 15万米
            生成器.最大距离 = 8000000; // 800万米（支持5个轮次）
            生成器.轮次数量 = 5; // 生成5个轮次
            生成器.十五万奖杯 = 50;
            生成器.四十万奖杯 = 100;
            生成器.八十万奖杯 = 150;
        ...
        public void 生成奖励规则()
        {
            ...
            // 按照80万一个轮回的等差数列公式生成规则
            // 每个轮回：15万=50奖杯，40万=100奖杯，80万=150奖杯
            // 生成指定轮次数量的规则
            for (int 轮回 = 0; 轮回 < 轮次数量; 轮回++)
            {
                int 轮回起始距离 = 轮回 * 800000;
                
                // 15万阶段（每个轮回内）
                int 距离15万 = 轮回起始距离 + 150000;
                if (距离15万 >= 最短距离 && 距离15万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离15万}", $"rule_{规则计数器:D4}", 十五万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成15万阶段规则: 距离>={距离15万}, 奖杯={十五万奖杯}");
                    }
                }
                
                // 40万阶段（每个轮回内）
                int 距离40万 = 轮回起始距离 + 400000;
                if (距离40万 >= 最短距离 && 距离40万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离40万}", $"rule_{规则计数器:D4}", 四十万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成40万阶段规则: 距离>={距离40万}, 奖杯={四十万奖杯}");
                    }
                }
                
                // 80万阶段（每个轮回内）
                int 距离80万 = 轮回起始距离 + 800000;
                if (距离80万 >= 最短距离 && 距离80万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离80万}", $"rule_{规则计数器:D4}", 八十万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
    ...
    public class 飞车关卡中级奖励规则生成器窗口 : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            生成器.十五万奖杯 = EditorGUILayout.IntField("15万阶段奖杯", 生成器.十五万奖杯);
            生成器.四十万奖杯 = EditorGUILayout.IntField("40万阶段奖杯", 生成器.四十万奖杯);
            生成器.八十万奖杯 = EditorGUILayout.IntField("80万阶段奖杯", 生成器.八十万奖杯);
