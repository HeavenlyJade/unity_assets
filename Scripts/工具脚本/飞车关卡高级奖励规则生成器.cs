...
namespace MiGame.Tools
{
    ...
    public class 飞车关卡高级奖励规则生成器 : MonoBehaviour
    {
        ...
        [Header("生成配置")]
        [Tooltip("最短距离（米）")]
        public int 最短距离 = 200000; // 20万米
        
        [Tooltip("最大距离（米）")]
        public int 最大距离 = 10000000; // 1000万米（支持5个轮次）
        
        [Tooltip("生成轮次数量")]
        public int 轮次数量 = 5;
        
        [Tooltip("20万阶段的奖杯数量")]
        public int 二十万奖杯 = 100;
        
        [Tooltip("50万阶段的奖杯数量")]
        public int 五十万奖杯 = 200;
        
        [Tooltip("100万阶段的奖杯数量")]
        public int 一百万奖杯 = 300;
        ...
        /// </summary>
        [MenuItem("Tools/关卡配置/飞车关卡高级奖杯奖励规则生成器")]
        ...
        public static void 快速生成飞车关卡高级奖励规则()
        {
            ...
            生成器.目标关卡配置 = 目标配置;
            生成器.最短距离 = 200000; // 20万米
            生成器.最大距离 = 10000000; // 1000万米（支持5个轮次）
            生成器.轮次数量 = 5; // 生成5个轮次
            生成器.二十万奖杯 = 100;
            生成器.五十万奖杯 = 200;
            生成器.一百万奖杯 = 300;
        ...
        public void 生成奖励规则()
        {
            ...
            // 按照100万一个轮回的等差数列公式生成规则
            // 每个轮回：20万=100奖杯，50万=200奖杯，100万=300奖杯
            // 生成指定轮次数量的规则
            for (int 轮回 = 0; 轮回 < 轮次数量; 轮回++)
            {
                int 轮回起始距离 = 轮回 * 1000000;
                
                // 20万阶段（每个轮回内）
                int 距离20万 = 轮回起始距离 + 200000;
                if (距离20万 >= 最短距离 && 距离20万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离20万}", $"rule_{规则计数器:D4}", 二十万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成20万阶段规则: 距离>={距离20万}, 奖杯={二十万奖杯}");
                    }
                }
                
                // 50万阶段（每个轮回内）
                int 距离50万 = 轮回起始距离 + 500000;
                if (距离50万 >= 最短距离 && 距离50万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离50万}", $"rule_{规则计数器:D4}", 五十万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成50万阶段规则: 距离>={距离50万}, 奖杯={五十万奖杯}");
                    }
                }
                
                // 100万阶段（每个轮回内）
                int 距离100万 = 轮回起始距离 + 1000000;
                if (距离100万 >= 最短距离 && 距离100万 <= 最大距离)
                {
                    var 规则 = 创建奖励规则($"distance>={距离100万}", $"rule_{规则计数器:D4}", 一百万奖杯);
                    生成的奖励规则.Add(规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成100万阶段规则: 距离>={距离100万}, 奖杯={一百万奖杯}");
                    }

                    // 同一触发点追加飞行币×300的奖励
                    var 飞行币规则 = 创建奖励规则自定义物品($"distance>={距离100万}", $"rule_{规则计数器:D4}", 飞行币物品类型, 300);
                    生成的奖励规则.Add(飞行币规则);
                    规则计数器++;
                    
                    if (输出调试信息)
                    {
                        Debug.Log($"生成100万阶段飞行币规则: 距离>={距离100万}, 飞行币=300");
                    }
                }
            }
    ...
    public class 飞车关卡高级奖励规则生成器窗口 : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            生成器.二十万奖杯 = EditorGUILayout.IntField("20万阶段奖杯", 生成器.二十万奖杯);
